...
def count_of_smiles(text, list_of_smiles):
    cur_count = 0
...
def lem_pos(text):
    ...
    text_lem = [wnl.lemmatize(word.lower(), pos=penn2morphy(tag)) for word, tag in pos_tag(nltk.word_tokenize(text))]

    return text_lem

def delete_stopwords(review, stoplist_combined):
